...
namespace MiniFPS {
    ...
    private:
        ...
        int GetTexX(const Vec2& startPos, const Vec2& intersectionPos, const Vec2& rayLength1D, const Vec2& rayUnitStepSize, const Vec2& rayDir, int textureSize);
        void CopyTextureToFrameTexture(SDLTextureBuffer buffer, const Texture& texture, IntPoint point, int w, int h);
        void DrawTexturedColumn(const Texture& texture, const Camera& camera, SDLTextureBuffer buffer, float distance, const Vec2& cell, int rayX, int texX);
